...
class _ChatListScreenState extends State<ChatListScreen>
  ...
  final Map<String, DateTime> _webPresenceSnapshotSuppressUntil =
      <String, DateTime>{};
  final Map<String, Timer> _webThreadHistoryGapFillTimers = <String, Timer>{};
  ...
  bool _syncChatTabActive = false;
  String? _lastEmbeddedRealtimeThreadIdsSig;
  String? _lastStandaloneListPresenceSig;
  final Map<String, bool> _webThreadPresenceOnly = <String, bool>{};
  static const int _maxConcurrentListChatSockets = 30;

  bool _chatListSelectionMode = false;
  final Set<String> _selectedChatThreadKeys = <String>{};
  ...
  Future<void> _embeddedSyncHook() async {
    if (!mounted) return;
    await _syncEmbeddedThreadRealtime(
      _safeVm.threads,
      chatTabIsSelected: _syncChatTabActive,
    );
  }
  ...
  Future<void> _pruneWebSocketsForThreadList(
    ...
    final ids = <String>{for (final t in threads) (t.targetUserId ?? '').trim()}
      ..remove('');
    final stale = <String>[];
  ...
  bool get _useApiForListDeletion => !kIsWeb || !widget.embedInBottomBar;

  void _exitChatListSelection() {
    if (!_chatListSelectionMode && _selectedChatThreadKeys.isEmpty) return;
    setState(() {
      _chatListSelectionMode = false;
      _selectedChatThreadKeys.clear();
    });
  }

  void _onThreadLongPressForSelection(ChatThreadPreview t) {
    final k = _threadKey(t);
    setState(() {
      if (!_chatListSelectionMode) {
        _chatListSelectionMode = true;
        _selectedChatThreadKeys.add(k);
      } else if (_selectedChatThreadKeys.contains(k)) {
        _selectedChatThreadKeys.remove(k);
        if (_selectedChatThreadKeys.isEmpty) {
          _chatListSelectionMode = false;
        }
      } else {
        _selectedChatThreadKeys.add(k);
  ...
  void _toggleThreadSelection(ChatThreadPreview t) {
    final k = _threadKey(t);
    setState(() {
      if (_selectedChatThreadKeys.contains(k)) {
        _selectedChatThreadKeys.remove(k);
        if (_selectedChatThreadKeys.isEmpty) {
          _chatListSelectionMode = false;
        }
  ...
  Future<void> _confirmAndDeleteSelectedChats(
    ...
    ChatListScreenViewModel model,
    List<ChatThreadPreview> filteredThreads,
  ) async {
    if (!_chatListSelectionMode || _selectedChatThreadKeys.isEmpty) return;

    final toRemove = filteredThreads
        .where((t) => _selectedChatThreadKeys.contains(_threadKey(t)))
        .toList();
    if (toRemove.isEmpty) {
      _exitChatListSelection();
      return;
    }

    final useApi = _useApiForListDeletion;
    final apiTargets = useApi
        ? toRemove
              .where((t) => (t.targetUserId ?? '').trim().isNotEmpty)
              .toList()
        : toRemove;

    if (useApi && apiTargets.isEmpty) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Selected chats cannot be deleted.')),
        );
      }
      return;
    }

    final deleteCount = useApi ? apiTargets.length : toRemove.length;
    final confirmed = await showAppDialog<bool>(
      context,
      title: 'Delete chats?',
      message:
          'Remove $deleteCount chat${deleteCount == 1 ? '' : 's'} from your chat list?',
      actions: [
        AppDialogAction(
          label: 'Cancel',
    ...
    if (useApi) {
      for (final t in apiTargets) {
        ...
        try {
          await model.deleteConversation(targetUserId: id);
        } catch (e) {
          if (context.mounted) {
      ...
      for (final t in apiTargets) {
        ChatScreenMessagesCache.invalidate((t.targetUserId ?? '').trim());
      }
    } else {
      await Future.wait(toRemove.map((t) => _disposeWebThread(_threadKey(t))));
      for (final t in toRemove) {
        _webThreadMessages.remove(_threadKey(t));
        ChatScreenMessagesCache.invalidate((t.targetUserId ?? '').trim());
      }
      ChatListScreenViewModel.removeThreadsBatch(toRemove);
      final removedIds = toRemove
          .map((t) => (t.targetUserId ?? '').trim())
          .where((id) => id.isNotEmpty)
          .toSet();
      final cur = (_selectedWebTargetUserId ?? '').trim();
      if (cur.isNotEmpty && removedIds.contains(cur)) {
  ...
  Widget _chatListMultiSelectToolbar(
    BuildContext context,
    ChatListScreenViewModel model,
    List<ChatThreadPreview> filteredThreads,
  ) {
    if (!_chatListSelectionMode) return const SizedBox.shrink();
    return Material(
      color: context.appColors.blue10,
      child: Padding(
        padding: context.padSym(h: 4, v: 2),
        child: Row(
          children: [
            IconButton(
              icon: const Icon(Icons.close),
              onPressed: _exitChatListSelection,
              tooltip: 'Cancel',
            ),
            Expanded(
              child: Text(
                '${_selectedChatThreadKeys.length} selected',
            ...
            IconButton(
              ...
              onPressed: _selectedChatThreadKeys.isEmpty
                  ? null
                  : () => unawaited(
  ...
  Widget _buildChatListThreadRow({
    required BuildContext context,
    required ChatListScreenViewModel model,
    required ChatThreadPreview t,
  }) {
    final targetUserId = (t.targetUserId ?? '').trim();
    final canDelete = targetUserId.isNotEmpty;
    final selectionKey = _threadKey(t);
    final isSelected = _selectedChatThreadKeys.contains(selectionKey);
    final useApi = _useApiForListDeletion;

    final Color rowBg = _chatListSelectionMode && isSelected
          ...
          Stack(
            ...
            children: [
              CircleAvatar(
                ...
                child: (t.avatarUrl ?? '').trim().isNotEmpty
                    ? ClipOval(
                        child: Image.network(
                          t.avatarUrl!.trim(),
                          width: context.w(44),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  () {
                    if (t.isOnline) return 'Online';
                    final iso = (t.lastSeenIso ?? '').trim();
                    if (iso.isEmpty) return 'Offline';
          ...
          Column(
            ...
            children: [
              ...
              if (t.unreadCount > 0) ...[
                ...
                Container(
                  padding: context.padSym(h: 7, v: 2),
                  decoration: BoxDecoration(
    ...
    Widget row = GestureDetector(
      onLongPress: () => _onThreadLongPressForSelection(t),
      onTap: () async {
        if (_chatListSelectionMode) {
    ...
    if (_chatListSelectionMode || !canDelete) return row;

    if (useApi) {
      return Dismissible(
        key: ValueKey<String>('chat-$targetUserId'),
        ...
        onDismissed: (_) async {
          try {
            await model.deleteConversation(targetUserId: targetUserId);
          } catch (e) {
    ...
    return Dismissible(
      key: ValueKey('chat_thread_${t.targetUserId}_${t.userName}'),
      direction: DismissDirection.endToStart,
      confirmDismiss: (_) async {
        final confirmed = await showAppDialog<bool>(
          context,
          title: 'Delete chat?',
          message: 'This will remove the chat thread locally.',
          actions: [
            AppDialogAction(
              label: 'Cancel',
              onPressed: (ctx) => Navigator.of(ctx).pop(false),
  ...
  Future<void> _bindWebThread(
    ...
    if (_webThreadServices.containsKey(key)) {
      final isPresence = _webThreadPresenceOnly[key] == true;
      if (presenceOnly) {
    ...
    _webThreadPresenceOnly[key] = presenceOnly;

    if (mounted && kIsWeb && widget.embedInBottomBar && !presenceOnly) {
    ...
    if (presenceOnly) {
      _attachListPresenceListeners(
        key: key,
        thread: thread,
        targetUserId: targetUserId,
        service: service,
        presenceOnlyBinding: true,
      );
      service.connect();
      return;
    }
    ...
    try {
      ...
      for (final item in history) {
        ...
        list.add(
          WebChatMessageItem(
            text: item.isDeleted
                ? (isMeHist ? '' : AppText.chatMessageWasDeleted)
                : item.content,
            time:
    ...
    _webMessageSubs[key] = service.onMessage.listen((msg) {
        ...
        if (dupIdx >= 0) {
          if (msg.isDeleted) {
            ...
            list[dupIdx] = ex.copyWith(
              isDeleted: true,
              text: ex.isMe ? '' : AppText.chatMessageWasDeleted,
              isPending: false,
              isFailed: false,
            );
            if (!isMine) {
      ...
      if (isMine) {
        ...
        final pendingIndex = list.indexWhere(
          (item) =>
              item.isMe &&
              item.isPending &&
              item.text.trim() == msg.content.trim(),
        );
        if (pendingIndex >= 0) {
          list[pendingIndex] = list[pendingIndex].copyWith(
            isPending: false,
            isFailed: false,
            messageId: msg.messageId.trim().isEmpty
                ? null
                : msg.messageId.trim(),
            readAt: rdDelMine.$1,
        ...
        list.add(
          WebChatMessageItem(
            text: msg.isDeleted ? AppText.chatMessageWasDeleted : msg.content,
            time: MaterialLocalizations.of(
              context,
            ).formatTimeOfDay(TimeOfDay.fromDateTime(msg.sentAt.toLocal())),
            isMe: false,
      ...
      if (!isMine && mid.isNotEmpty) {
        _webSendReadReceiptOnce(key: key, service: service, rawMessageId: mid);
      }
      _upsertPreviewFromWebRealtime(
        thread: thread,
        targetUserId: targetUserId,
  ...
  void _attachListPresenceListeners({
    required String key,
    required ChatThreadPreview thread,
    required String targetUserId,
    required MatchChatService service,
    required bool presenceOnlyBinding,
  }) {
    ...
    _webConnectedSubs[key] = service.onConnected.listen((_) {
      _webPresenceSnapshotSuppressUntil[key] = DateTime.now().add(
        const Duration(seconds: 4),
      );
      if (presenceOnlyBinding) return;
  ...
  Future<void> _disposeWebThread(String key) async {
    ...
    _webThreadPresenceOnly.remove(key);
    _webThreadServices.remove(key)?.dispose();
  }

  List<ChatThreadPreview> _pickThreadsForListPresence(
    List<ChatThreadPreview> threads, {
    String? ensureTargetUserId,
  }) {
    final withId = threads
        .where((t) => (t.targetUserId ?? '').trim().isNotEmpty)
        .toList();
    final ensure = (ensureTargetUserId ?? '').trim();
    ChatThreadPreview? pinned;
    final rest = <ChatThreadPreview>[];
    for (final t in withId) {
      if (ensure.isNotEmpty && (t.targetUserId ?? '').trim() == ensure) {
        pinned = t;
      } else {
        rest.add(t);
      }
    }
    rest.sort(ChatListScreenViewModel.compareThreadsByRecency);
    final out = <ChatThreadPreview>[?pinned, ...rest];
    if (out.length <= _maxConcurrentListChatSockets) return out;
    return out.sublist(0, _maxConcurrentListChatSockets);
  }

  Future<void> _syncStandaloneListPresence(
    List<ChatThreadPreview> threads,
  ) async {
    if (!mounted || widget.embedInBottomBar) return;
    ...
    if (activeFullscreen.isNotEmpty) {
      await _disposeWebThread('user:$activeFullscreen');
    }

    final picked = _pickThreadsForListPresence(threads);
    for (final t in picked) {
      final id = (t.targetUserId ?? '').trim();
      if (id.isEmpty || ChatListRealtimeCoordinator.matchesFullscreen(id)) {
        continue;
      }
      await _bindWebThread(t, presenceOnly: true);
    }
  }

  Future<void> _syncEmbeddedThreadRealtime(
    List<ChatThreadPreview> threads, {
    required bool chatTabIsSelected,
  }) async {
    await _pruneWebSocketsForThreadList(threads);

    final activeFullscreen =
        (ChatListRealtimeCoordinator.fullScreenDirectTargetUserId ?? '').trim();
    final activeFullscreenKey = activeFullscreen.isEmpty
        ? null
        : 'user:$activeFullscreen';
    ...
    final w = kIsWeb && mounted ? MediaQuery.sizeOf(context).width : 0.0;
    final desktopSplit = kIsWeb && w >= _webCompactBreakpoint;
    ...
    // Embedded web: avoid N concurrent list sockets. Desktop split keeps one
    // full connection for the open thread only; narrow layout uses fullscreen
    // [ChatScreen] for the active thread (no list-level WS here).
    if (!chatTabIsSelected) {
    ...
    if (desktopSplit) {
      ...
      final selectedThread = _findThreadByTargetUserId(sel);
      if (selectedThread == null) {
        await _disposeWebThreadsExcept(null);
        return;
      }
      final keepKey = 'user:$sel';
      await _disposeWebThreadsExcept(keepKey);
      await _bindWebThread(selectedThread, presenceOnly: false);
      return;
    }
  ...
  Future<void> _openEmbeddedDirectChatFromRouteArgs(
    ChatRouteArgs selected,
  ) async {
  ...
  Future<void> _catchUpOpenWebThreadsOnResume() async {
    ...
    for (final entry in Map<String, MatchChatService>.from(
      _webThreadServices,
    ).entries) {
      final key = entry.key;
  ...
  void reassemble() {
    super.reassemble();
    // Hot reload keeps static thread/unread state; re-fetch so badges match API.
    if (kDebugMode) {
      unawaited(_safeVm.refreshDirectChats());
    }
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    switch (state) {
      case AppLifecycleState.resumed:
        _lastEmbeddedRealtimeThreadIdsSig = null;
        _lastStandaloneListPresenceSig = null;
        unawaited(_catchUpOpenWebThreadsOnResume());
        if (widget.embedInBottomBar) {
          AppLogger.debug(
            'ChatListScreen lifecycle resumed → re-sync embedded realtime',
            tag: 'AppLifecycle',
          );
          unawaited(_embeddedSyncHook());
        }
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (!mounted) return;
          setState(() {});
      ...
      case AppLifecycleState.hidden:
        AppLogger.debug(
          'ChatListScreen lifecycle $state → dispose all list/chat WS',
          tag: 'AppLifecycle',
        );
          ...
          Expanded(
            child:
                ...
                ? Column(
                    children: [
                      Padding(
                        padding: context.padSym(h: 20, v: 0),
                        child: _chatListMultiSelectToolbar(
                          context,
                          model,
                          filteredThreads,
                        ),
                        ...
                        child: filteredThreads.isNotEmpty
                            ? ListView.separated(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return _buildChatListThreadRow(
                                    context: context,
                                    model: model,
                                    t: t,
                                  );
  ...
  Widget build(BuildContext context) {
    final bottomBarSelectedIndex = widget.embedInBottomBar
        ? context.select<BottomBarScreenViewModel, int>(
            (vm) => vm.selectedIndex,
          )
        : null;
    final isActiveTab = bottomBarSelectedIndex == _chatTabIndex;
    final embeddedChatTabBecameActive =
        widget.embedInBottomBar && isActiveTab && !_wasActiveBottomBarTab;
    final embeddedChatTabBecameInactive =
        widget.embedInBottomBar && !isActiveTab && _wasActiveBottomBarTab;
    _syncChatTabActive = widget.embedInBottomBar && isActiveTab;

    if (embeddedChatTabBecameInactive) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
        AppLogger.debug(
          'Chat tab deselected (embedded) → dispose list WS, clear ephemeral UI',
          tag: 'AppLifecycle',
        );
        unawaited(_embeddedSyncHook());
        ...
        builder: (context, model, _) {
          ...
          if (widget.embedInBottomBar) {
            ...
            if (lifecycle == null || lifecycle == AppLifecycleState.resumed) {
              ...
              final sig = ids.join('\u001f');
              final compositeSig = '$sig|tab:${isActiveTab ? '1' : '0'}';
              if (compositeSig != _lastEmbeddedRealtimeThreadIdsSig ||
                  embeddedChatTabBecameActive ||
                  embeddedChatTabBecameInactive) {
                _lastEmbeddedRealtimeThreadIdsSig = compositeSig;
                WidgetsBinding.instance.addPostFrameCallback((_) async {
                  if (!mounted || !context.mounted) return;
                  await _syncEmbeddedThreadRealtime(
                    model.threads,
                    chatTabIsSelected: isActiveTab,
                  );
                });
              }
            }
          ...
          if (!widget.embedInBottomBar) {
            ...
            final sig = ids.join('\u001f');
            if (sig != _lastStandaloneListPresenceSig) {
              _lastStandaloneListPresenceSig = sig;
              WidgetsBinding.instance.addPostFrameCallback((_) async {
                if (!mounted || !context.mounted) return;
                await _syncStandaloneListPresence(model.threads);
              });
            }
          }
          ...
          for (var i = 0; i < allThreads.length; i++) {
            ...
            final haystack = '${t.userName} ${t.lastMessage}'.toLowerCase();
                  ...
                  builder: (context, constraints) {
                    ...
                    return WebChatContent(
                      ...
                      selectedThreadIndex:
                          ...
                          : null,
                      activeMessages: (_selectedWebTargetUserId != null)
                          ? List<WebChatMessageItem>.unmodifiable(
                              _webThreadMessages['user:${(_selectedWebTargetUserId ?? '').trim()}'] ??
                      ...
                      chatListSelectionMode: _chatListSelectionMode,
                      selectedThreadKeys: _selectedChatThreadKeys,
                      onExitChatListSelection: _exitChatListSelection,
                      onDeleteSelectedThreads: () => unawaited(
                        _confirmAndDeleteSelectedChats(
                          context,
                          model,
                          filteredThreads,
                      ...
                      onThreadSelectionTap: (index) {
                        if (index < 0 || index >= filteredThreads.length) {
                          return;
                        }
                        _toggleThreadSelection(filteredThreads[index]);
                      },
                      onThreadLongPressForSelection: (index) {
                        if (index < 0 || index >= filteredThreads.length) {
                          return;
                        }
                        _onThreadLongPressForSelection(filteredThreads[index]);
                      },
                      onPeerProfileTap: () {
                        final thread = _findThreadByTargetUserId(
                          _selectedWebTargetUserId,
                        );
                        if (thread == null) return;
                        final userId = (thread.targetUserId ?? '').trim();
                        if (userId.isEmpty) return;
                        final raw = thread.userName.trim();
                        final displayName = raw.isNotEmpty ? raw : 'User';
                        Navigator.pushNamed(
                          context,
                      ...
                      onThreadSelected: (index) {
                        ...
                        setState(
                          () =>
                              _selectedWebTargetUserId = selected?.targetUserId,
                        );
                        if (selected != null) {
                          ChatListScreenViewModel.markRead(
                      ...
                      onSendMessage: () {
                        ...
                        final activeThread = _findThreadByTargetUserId(
                          _selectedWebTargetUserId,
                        );
                    ...
                    children: [
                      ...
                      if (!widget.embedInBottomBar)
                        Padding(
                          ...
                          child: AppBarWidget(
                            showBackButton: true,
                            onTapFirst: _chatListSelectionMode
                                ? _exitChatListSelection
                                : () => Navigator.pop(context),
                            title: _chatListSelectionMode
                                ? null
                                : AppText.sportFinding,
                            centerWidget: _chatListSelectionMode
                                ? Center(
                                    child: NormalText(
                                      titleText:
                                          '${_selectedChatThreadKeys.length} selected',
                                ...
                                : null,
                            trailingActions: _chatListSelectionMode
                                ? [
                                    IconButton(
                                      icon: const Icon(Icons.delete_outline),
                                      onPressed: _selectedChatThreadKeys.isEmpty
          ...
          return PopScope(
            canPop: !_chatListSelectionMode,
            onPopInvokedWithResult: (didPop, result) {
              if (didPop) return;
              if (_chatListSelectionMode) _exitChatListSelection();
            },
            child: Scaffold(
              body: content,
              floatingActionButton: _chatListSelectionMode
                  ? null
